...
%typemap(in) (type_name) {
    if (PyList_Check($input)) {
        ...
        if (count > 0) {
            auto itemTypeValid = [](PyObject* item) {
                if (std::is_floating_point<$1_basetype::value_type>::value) {
                    return static_cast<bool>(PyFloat_Check(item));
                }
                if (std::is_integral<$1_basetype::value_type>::value) {
                    return static_cast<bool>(PyLong_Check(item) || PyInt_Check(item));
                }
            ...
            if (!itemTypeValid(item)) {
                SWIG_exception(SWIG_TypeError, "wrong element type");
            }
        }

        using value_type = std::remove_cv<$1_basetype::value_type>::type;
        auto ptr = reinterpret_cast<value_type*>(malloc(count * sizeof(value_type)));
