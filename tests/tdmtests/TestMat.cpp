...
TEST(MatTestConstruction, ConstructSquareFromRows) {
    ...
    tdm::mat4<int> m = tdm::mat4<int>::from_rows(v, v, v, v);
    ASSERT_EQ(m[0], tdm::vec4<int>(1, 2, 3, 4));
...
TEST(MatTestConstruction, ConstructNonSquareFromRows) {
    tdm::vec2<int> v{1, 2};
    tdm::mat<4, 2, int> m = tdm::mat<4, 2, int>::from_rows(v, v, v, v);
...
TEST(MatTestConstruction, ConstructSquareFromCols) {
    tdm::vec4<int> v{1, 2, 3, 4};
    tdm::mat4<int> m = tdm::mat4<int>::from_columns(v, v, v, v);
...
TEST(MatTestConstruction, ConstructNonSquareFromCols) {
    tdm::vec4<int> v{1, 2, 3, 4};
    tdm::mat<4, 2, int> m = tdm::mat<4, 2, int>::from_columns(v, v);
...
TEST_F(MatTestOperators, TestCompoundAssignmentDivideInitializer) {
    ...
    tdm::mat4<float> expected{tdm::vec4<float>{1.0f, -5.96046e-08f, 2.38419e-07f, -2.38419e-07f},
...
TEST_F(MatTestOperators, TestMultiplyMat4x3Mat3x4) {
    tdm::mat<4, 3, int> m4x3{1, 2, 3, 5, 6, 7, 9, 10, 11, 13, 14, 15};
    tdm::mat<3, 2, int> m3x2{3, 4, 7, 8, 11, 12};
...
TEST_F(MatTestMembers, TestApplyMat3) {
    std::size_t count{};
    tdm::mat3<int> m1{tdm::vec3<int>{1, 2, 3}, tdm::vec3<int>{4, 5, 6}, tdm::vec3<int>{7, 8, 9}};
